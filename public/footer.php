
    <footer>
        <div id="footer">
            <div class="container-fluid footerFluid">
                <div class="row classRowFooter" >
                    <div class="col-xs-4">
                        <a href="contact.php"><img src="images/contact-footer.png" class="imgFooter" alt=""> </a>
                                <br>
                        <a href="contact.php"><h2 class="footertext">Nous contacter</h2> </a>
                                <br/>
                    </div>
                    <div class="col-xs-4">
                       <a href="contact.php"> <img src="images/adresse-footer.png" class="imgFooter" alt=""> </a>
                                <br>
                        <a href="contact.php"><h2 class="footertext">31, rue St Simon <br/> 69009 Lyon</h2></a>

                                <br/>
                    </div>
                    <div class="col-xs-4">
                        <a href="contact.php"><img src="images/telephone-footer.png" class="imgFooter" alt=""> </a>
                                 <br>
                        <a href="contact.php"><h2 class="footertext">04 27 11 86 00</h2> </a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-12">
                    <p class="mentions">@2016 Neoxi - MENTIONS LEGALES Site réalisé par les élèves de la Wild Code School</p>
                </div>
            </div>
        </div>
    </footer>


